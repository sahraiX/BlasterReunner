    ...
    public class GitData
    {
        ...
        private void FetchInformations()
        {
            ...
            if (IsGitAvailable)
            {
                ...
                {
                    CustomLog.LogError($"Error parsing version \"{FullGitTagResult}\" : {e.Message}");
                    CustomLog.LogError($"Did you tag match the System.Version format : \"X.Y\" or \"X.Y.Z\" where X is the major int, Y the minor int and optionaly Z the revision int ?");
                    CustomLog.LogError("From Microsoft doc : The version parameter can contain only the components major, minor, build, and revision, in that order, and all separated by periods. There must be at least two components, and at most four. The first two components are assumed to be major and minor. The value of unspecified components is undefined. (https://learn.microsoft.com/en-us/dotnet/api/system.version.-ctor?view=net-7.0#system-version-ctor(system-string))");
                }
        ...
        private void ParseDescribeResult()
        {
            ...
            // Get the hash
            int startIndex = FullGitTagResult.LastIndexOf('-') + 2;
            CommitHash = FullGitTagResult.Substring(startIndex, FullGitTagResult.Length - startIndex);
