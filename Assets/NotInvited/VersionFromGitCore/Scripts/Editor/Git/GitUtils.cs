...
namespace NotInvited.VersionFromGit.Editor.Git
{
    ...
    public static class GitUtils
    {
        ...
        {
            string args = $@"describe --tags --long --match ""{tagFormat}*""";

            var cmdResult = GitCmd(args, false);

            bool tagFound = cmdResult.success;
            
            // If not tage found it retrieve the abbreviated hash of the last commit
            string tag = cmdResult.success == true ? cmdResult.output : GetAbbreviatedHashOfLastCommit();

            return (tagFound, tag);
        }

        /// <summary>
        /// Get the current commit abbreviated hash
        /// </summary>
        /// <returns></returns>
        public static string GetAbbreviatedHashOfLastCommit()
        {
            string args = $@"--no-pager log -1 --format=""%h""";
